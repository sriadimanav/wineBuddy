    ...
    },
    {
      ...
      detect: () => {
        return (
          ...
          process.env &&
          DEVELOPMENT_INDICATORS.nodeEnvValues.includes(process.env.NODE_ENV?.toLowerCase() || '')
        );
      },
    },
    {
      ...
      detect: () => {
        ...
        const isDevHostname =
          ...
          DEVELOPMENT_INDICATORS.hostnamePatterns.some(pattern => pattern.test(hostname));
    ...
    try {
      ...
      for (const strategy of sortedStrategies) {
        try {
          ...
          console.warn(`Environment detection strategy "${strategy.name}" failed:`, error);
          continue;
        }
      }
